...
(function() {
    ...
    const GradientController = {
        ...
        init() {
            try {
                ...
                console.error('[Gradient Controller] Initialization failed:', error);
            }
        },
        ...
        updateGradient() {
            try {
                ...
                // Calculate gradient expansion factor
                // At top of page: 0, at bottom of page: 1
                const gradientProgress = scrollProgress;

                // Update CSS custom properties
                document.documentElement.style.setProperty('--gradient-progress', gradientProgress);
                document.documentElement.style.setProperty('--viewport-height', `${windowHeight}px`);
                document.documentElement.style.setProperty('--page-height', `${documentHeight}px`);

                // Optional: Log for debugging (remove in production)
                if (window.location.search.includes('debug=gradient')) {
                    console.log('[Gradient Controller] Progress:', {
                        scrollTop,
                ...
                console.error('[Gradient Controller] Update failed:', error);
            }
        }
    };
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', function() {
            ...
            initTOC();
            fixCommonIssues();
            GradientController.init(); // Initialize gradient controller
